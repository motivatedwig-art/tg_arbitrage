export class BybitAdapter {
    ...
    async getTickers() {
        ...
        try {
            ...
            const tickers = (data.result?.list || [])
                .filter((ticker) => ticker.bid1Price && ticker.ask1Price)
                .map((ticker) => ({
    ...
    async getOrderBook(symbol) {
        ...
        try {
            ...
            return {
                symbol,
                bids: data.result.b.map((bid) => [parseFloat(bid[0]), parseFloat(bid[1])]),
                asks: data.result.a.map((ask) => [parseFloat(ask[0]), parseFloat(ask[1])]),
                timestamp: Date.now(),
                exchange: 'bybit'
            };
        }
