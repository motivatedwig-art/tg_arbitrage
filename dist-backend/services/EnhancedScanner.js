...
export class EnhancedScanner {
    constructor() {
        ...
        this.exchangeSymbolsCache = new Map();
        this.SYMBOLS_CACHE_DURATION = 3600000; // 1 hour
        this.exchangeManager = ExchangeManager.getInstance();
        this.calculator = new ArbitrageCalculator(parseFloat(process.env.MIN_PROFIT_THRESHOLD || '0.5'), parseFloat(process.env.MAX_PROFIT_THRESHOLD || '50'), parseFloat(process.env.MIN_VOLUME_THRESHOLD || '10'));
     ...
     */
    enhanceTickersWithBlockchain(allTickers) {
    ...
    detectBlockchainFromSymbol(symbol) {
        ...
        if (cleanSymbol.includes('SOL') || cleanSymbol.includes('WSOL'))
            return 'solana';
        if (cleanSymbol.includes('TRX') || cleanSymbol.includes('TRON'))
            return 'tron';
        if (cleanSymbol.includes('BNB') || cleanSymbol.includes('WBNB') || cleanSymbol === 'BNB')
            return 'bsc';
        if (cleanSymbol.includes('MATIC') || cleanSymbol.includes('WMATIC') || cleanSymbol === 'MATIC')
            return 'polygon';
        if (cleanSymbol.includes('ARB') && !cleanSymbol.includes('BARB'))
            return 'arbitrum';
        if (cleanSymbol === 'OP' || (cleanSymbol.includes('OP') && cleanSymbol.length <= 8))
            return 'optimism';
        if (cleanSymbol.includes('AVAX') || cleanSymbol.includes('WAVAX') || cleanSymbol === 'AVAX')
            return 'avalanche';
        if (cleanSymbol.includes('TON') || cleanSymbol === 'TON')
            return 'ton';
        if (cleanSymbol.includes('APT') || cleanSymbol === 'APT')
            return 'aptos';
        if (cleanSymbol.includes('SUI') || cleanSymbol === 'SUI')
            return 'sui';
        if (cleanSymbol.includes('NEAR') || cleanSymbol === 'NEAR')
            return 'near';
        if (cleanSymbol.includes('ATOM') || cleanSymbol === 'ATOM')
            return 'cosmos';
        if (cleanSymbol.includes('DOT') || cleanSymbol === 'DOT')
            return 'polkadot';
        if (cleanSymbol.includes('ADA') || cleanSymbol === 'ADA')
            return 'cardano';
        // 3. Native chain tokens
        if (cleanSymbol === 'BTC' || cleanSymbol.includes('BTC'))
            return 'bitcoin';
        if (cleanSymbol === 'XRP' || cleanSymbol.includes('XRP'))
            return 'ripple';
        if (cleanSymbol === 'XLM' || cleanSymbol.includes('XLM'))
            return 'stellar';
        if (cleanSymbol === 'DOGE' || cleanSymbol.includes('DOGE'))
            return 'dogecoin';
        if (cleanSymbol === 'LTC' || cleanSymbol.includes('LTC'))
            return 'litecoin';
