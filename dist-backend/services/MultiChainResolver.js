...
export class MultiChainResolver {
     ...
     */
    async resolveTokenChains(symbol, exchangeData) {
     ...
     */
    consolidateChains(symbol, exchangeData) {
        ...
        for (const exchangeInfo of exchangeData) {
            for (const network of exchangeInfo.networks) {
                ...
                if (network.depositEnabled)
                    chainInfo.depositEnabled = true;
                if (network.withdrawEnabled)
                    chainInfo.withdrawEnabled = true;
     ...
     */
    determinePrimaryChain(symbol, exchangeData, availableChains) {
     ...
     */
    isNativeToken(symbol, blockchain) {
     ...
     */
    isWrappedToken(symbol, blockchain) {
     ...
     */
    getNativeChainForSymbol(symbol) {
        const upperSymbol = symbol.toUpperCase();
        const nativeMap = {
            'BTC': 'bitcoin',
            'ETH': 'ethereum',
     ...
     */
    calculateLiquidityScore(chain, exchangeData) {
     ...
     */
    calculateConfidence(exchangeData, availableChains) {
        let score = 0;
