...
export class ClaudeCommandHandler {
  ...
  private async ensureUser(msg: TelegramBot.Message): Promise<any> {
    const telegramId = msg.from!.id;
    let user = await this.db.getUserModel().findByTelegramId(telegramId);
    if (!user) {
      const defaultPreferences = {
        language: (msg.from?.language_code === 'en' ? 'en' : 'ru') as 'en' | 'ru',
        notifications: true,
  ...
  private async getTopOpportunities(limit: number = 3): Promise<ArbitrageOpportunity[]> {
    try {
    ...
    try {
      ...
      if (!opportunity) {
        await this.bot.sendMessage(msg.chat.id,
          `❌ Возможность ${symbol} на ${chain} не найдена`
        );
        return;
      }
...
💧 Ликвидность: $${opportunity.liquidity_usd.toLocaleString()}`;
  ...
  private async handleAI(msg: TelegramBot.Message): Promise<void> {
    try {
      const user = await this.ensureUser(msg);
      const lng = user.preferences.language;
      ...
      await this.bot.sendMessage(msg.chat.id,
        `🤖 <b>AI Анализ топ-3 возможностей</b>\n\nАнализирую с помощью Claude AI...`,
        { parse_mode: 'HTML' }
      );
...
💰 Примерная стоимость: $${costMetrics.estimated_cost.toFixed(4)}`;
  ...
  async getAIAnalysisForOpportunity(opportunity: any): Promise<string> {
    try {
      // Convert to Claude format
      const claudeOpportunity: ArbitrageOpportunity = {
  ...
  getCostMetrics(): { total_requests: number; cached_requests: number; estimated_cost: number; last_reset: number } {
