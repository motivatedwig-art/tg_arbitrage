...
export class ClaudeCommandHandler {
  ...
  private async handleContractLookup(msg: TelegramBot.Message, match: RegExpExecArray | null): Promise<void> {
    ...
    if (!symbol) {
      await this.bot.sendMessage(
        msg.chat.id,
        `❌ ${lng === 'en' ? 'Please specify a token symbol. Example: /contract ETH' : 'Пожалуйста, укажите символ токена. Пример: /contract ETH'}`
      );
      return;
    }
    ...
    try {
      ...
      if (!opportunity) {
        await this.bot.sendMessage(
          msg.chat.id,
          `⚠️ Данные по ${symbol} пока отсутствуют в базе. Подождите следующего сканирования.`
        );
        return;
      }
      ...
      if (!record || (!record.contractAddress && !record.chainId && !record.chainName)) {
        await this.bot.sendMessage(
          msg.chat.id,
          `❌ Не удалось получить контрактные данные для ${symbol}. Попробуйте позже.`
        );
        return;
      }
